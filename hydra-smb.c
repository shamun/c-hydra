...
int HashLM(unsigned char **lmhash, unsigned char *pass, unsigned char *challenge)
{
  ...
  if (hashFlag == 1) {
    ...
    if (*p == '\0') {
      ...
      hydra_report(stderr, "Found \"NO PASSWORD\" for LM Hash.\n");
      ...
      hydra_report(stderr, "Convert ASCII PwDump LM Hash (%s).\n", p);
      for (i = 0; i < 16; i++) {
        HexValue = 0x0;
        for (j = 0; j < 2; j++) {
...
*/  
  ...
  {
    if (verbose)
      hydra_report(stderr, "Setting Negotiate Protocol Response for LM.\n");
  ...
  */
  switch (rbuf[39])
  {
    case 0x01:
      ...
      hydra_report(stderr, "Server requested PLAINTEXT password.\n");
      security_mode = PLAINTEXT;
      
      if (hashFlag == 1)
      {
        hydra_report(stderr, "Server requested PLAINTEXT password. HASH password mode not supported for this configuration.\n");
        return 3;
      }
      if (hashFlag == 2)
      {
        hydra_report(stderr, "Server requested PLAINTEXT password. MACHINE password mode not supported for this configuration.\n");
        return 3;
      }      
      break;
    case 0x03:
      if (verbose)
        hydra_report(stderr, "Server requested ENCRYPTED password without security signatures.\n");
      security_mode = ENCRYPTED;     
      break;
    case 0x07:
    case 0x0F:
      if (verbose)
        hydra_report(stderr, "Server requested ENCRYPTED password.\n");
      security_mode = ENCRYPTED;
      break;
    default:
      if (verbose)
        hydra_report(stderr, "Unknown security mode request: %2.2X. Proceeding using ENCRYPTED password mode.\n", rbuf[39]);
      security_mode = ENCRYPTED;
      break;
  }
...
unsigned long SMBSessionSetup(int s, char* szLogin, char* szPassword, char *miscptr)
{
  ...
  if (security_mode == ENCRYPTED)
  {
    ...
    {
      if (verbose)
        hydra_report(stderr, "Attempting LM password authentication.\n");
    ...
    {
      if(verbose)
        hydra_report(stderr, "Attempting NTLM password authentication.\n");
    ...
    {
      if (verbose)
        hydra_report(stderr, "Attempting LMv2 password authentication.\n");
    ...
    {
      if (verbose)
        hydra_report(stderr,"Attempting LMv2/NTLMv2 password authentication.\n");
      ...
      if (verbose)
        hydra_report(stderr, "HashNTLMv2 response length: %d\n", iByteCount);
  ...
  {
    if (verbose)
    hydra_report(stderr, "Attempting PLAINTEXT password authentication.\n");
...
iByteCount = strlen(szPassword) + 1;
  ...
  {
    hydra_report(stderr, "Security_mode was not properly set. This should not happen.\n");
    return -1;
  }
  ...
  if (verbose)
    hydra_report(stderr, "Set NBSS header length: %2.2X\n", buf[3]);

  /* Set data byte count */
  buf[iOffset - 2] = iByteCount;
  if (verbose)
    hydra_report(stderr, "Set byte count: %2.2X\n", buf[57]);
...
start_smb(int s, char *ip, int port, unsigned char options, char *miscptr, FILE * fp)
{
  ...
  if (verbose)
    hydra_report(stderr, "SMBSessionRet: %8.8X SMBerr: %4.4X SMBaction: %2.2X\n", (unsigned int)SMBSessionRet, SMBerr, SMBaction);
  ...
  if (SMBerr == 0x000000) {         /* success */
    ...
    if (verbose)
      hydra_report(stderr, "Valid password, GPO Disabling Remote Connections Using NULL Passwords\n");
    hydra_report_found_host(port, ip, "smb", fp);
    hydra_completed_pair_found();
  } else if (SMBerr == 0x00015B) {  /* Valid password, GPO "Deny access to this computer from the network" */
    if (verbose)
      hydra_report(stderr, "Valid password, GPO Deny access to this computer from the network\n");
    hydra_report_found_host(port, ip, "smb", fp);
    hydra_completed_pair_found();
  } else if (SMBerr == 0x000193) {  /* Valid password, account expired  */
    if (verbose)
      hydra_report(stderr, "Valid password, account expired\n");
    hydra_report_found_host(port, ip, "smb", fp);
    hydra_completed_pair_found();
  } else if (SMBerr == 0x000224) {  /* Valid password, account expired  */
    if (verbose)
      hydra_report(stderr, "Valid password, password expired and must be changed on next logon\n");
    hydra_report_found_host(port, ip, "smb", fp);
    hydra_completed_pair_found();
