/*
 * hydra v7.2 (c) 2001-2012 by van Hauser / THC <vh@thc.org>
 * http://www.thc.org
 *
...
// ADD NEW SERVICES HERE
char *SERVICES =
  "afp cisco cisco-enable cvs firebird ftp[s] http[s]-{head|get} http[s]-{get|post}-form http-proxy http-proxy-urlenum icq imap irc ldap2 ldap3[-{cram|digest}md5] mssql mysql ncp nntp oracle oracle-listener oracle-sid pcanywhere pcnfs pop3 postgres rdp rexec rlogin rsh sapr3 sip smb smtp smtp-enum snmp socks5 ssh svn teamspeak telnet vmauthd vnc xmpp";
...
void help() {
  printf("Syntax: %s [[[-l LOGIN|-L FILE] [-p PASS|-P FILE]] | [-C FILE]] [-e nsr]" " [-o FILE] [-t TASKS] [-M FILE [-T TASKS]] [-w TIME] [-W TIME] [-f] [-s PORT]"
#ifdef HAVE_MATH_H
         " [-x MIN:MAX:CHARSET]"
#endif
  ...
  printf("  OPT       some service modules need special input (use -U to see module help)\n");

  printf("\n%s is a tool to guess/crack valid login/password pairs - usage only allowed\nfor legal purposes! Find the newest version at %s :-)\n", PROGRAM, RESOURCE);
  ...
  printf("\nExamples:\n  %s -l john -p doe 192.168.0.1 ftp\n", prg);
  printf("  %s -L user.txt -p secret 192.168.0.1 imap PLAIN\n", prg);
  printf("  %s -l admin -P pass.txt http-proxy://192.168.0.1\n", prg);
  printf("  %s -C defaults.txt -6 imap://[fe80::2c:31ff:fe12:ac11]:143/PLAIN\n", prg);
  exit(-1);
}
  ...
  if (hydra_options.service) {
    ...
    if (!find && (strcmp(hydra_options.service, "http-proxy") == 0)) {
      printf("Module http-proxy is optionally taking the page to authenticate at.\n"
             "Default is http://www.microsoft.com/)\n" "Basic, DIGEST-MD5, NTLM auths are supported and negociated automatically.\n\n");
      find = 1;
    }
...
void hydra_restore_write(int print_msg) {
  ...
  for (j = 0; j < hydra_brains.targets; j++)
    if (hydra_targets[j]->done != 1) {
      ...
      fprintf(f, "%s\n%d\n%d\n", hydra_targets[j]->target == NULL ? "" : hydra_targets[j]->target, hydra_targets[j]->login_ptr - login_ptr, hydra_targets[j]->pass_ptr - pass_ptr);
...
void kill_children(int signo) {
  ...
  if (verbose)
    printf("[ERROR] Received signal %d, going down ...\n", signo);
  if (process_restore == 1)
    ...
    if (k <= 1) {
      if (hydra_targets[target_no]->fail_count >= MAXFAIL + hydra_options.tasks * hydra_targets[target_no]->ok) {
        ...
        if (hydra_targets[target_no]->done == 0 && hydra_options.max_use == hydra_targets[target_no]->failed) {
          ...
          fprintf(stderr, "Error: Too many connect errors to target, disabling %s://%s\n", hydra_options.service, hydra_targets[target_no]->target);
        }
...
int hydra_send_next_pair(int target_no, int head_no) {
  int done = 0, is_redo = 0, buflen, i, j, dont = 0;
  char buf[MAXBUF];

  if (loop_cnt++ > 16) {
    if (debug)
      printf("[DEBUG] too many loops in send_next_pair, returning -1\n");
    return -1;
  }

  if (debug)
    printf("[DEBUG] send_next_pair target %d, head %d, redo %d, redo_state %d, pass_state %d. loop_mode %d, curlogin %s, curpass %s, tlogin %s, tpass %s, logincnt %lu/%lu, passcnt %lu/%lu\n",
      target_no, head_no, hydra_heads[head_no]->redo, hydra_targets[target_no]->redo_state, hydra_targets[target_no]->pass_state, hydra_options.loop_mode, hydra_heads[head_no]->current_login_ptr, hydra_heads[head_no]->current_pass_ptr, hydra_targets[target_no]->login_ptr, hydra_targets[target_no]->pass_ptr, hydra_targets[target_no]->login_no, hydra_brains.countlogin, hydra_targets[target_no]->pass_no, hydra_brains.countpass);
    ...
    if (hydra_targets[target_no]->redo_state > 0) {
      ...
      if (hydra_options.loop_mode == 0) { // one user after another
        if (hydra_targets[target_no]->login_no < hydra_brains.countlogin) {
          ...
          if (hydra_targets[target_no]->pass_state == 0 && done == 0) {
          ...
          if (hydra_targets[target_no]->pass_state == 1 && done == 0) {
          ...
          if (hydra_targets[target_no]->pass_state == 2 && done == 0) {
            // small check that there is a login name (could also be emtpy) and if we already tried empty password it would be a double
          ...
          if (hydra_targets[target_no]->pass_state == 3 && done == 0) {
...
//              hydra_targets[target_no]->pass_ptr++;
        ...
        if (hydra_targets[target_no]->pass_no < hydra_brains.countpass) {
          ...
          if (hydra_targets[target_no]->pass_state == 3 && dont == 0) {
    ...
    if (debug)
      printf("[DEBUG] send_next_pair done %d, pass_state %d, clogin %s, cpass %s, tlogin %s, tpass %s\n",
       done, hydra_targets[target_no]->pass_state, hydra_heads[head_no]->current_login_ptr, hydra_heads[head_no]->current_pass_ptr, hydra_targets[target_no]->login_ptr, hydra_targets[target_no]->pass_ptr);
    
    // no pair? then we go for redo state
    if (!done && hydra_targets[target_no]->redo_state == 0 && hydra_targets[target_no]->redo > 0) {
      if (debug)
  ...
  if (!done || hydra_targets[target_no]->skipcnt >= hydra_brains.countlogin) {
    ...
    if (hydra_targets[target_no]->skipcnt > 0) {
      j = 0;
      for (i = 0; i < hydra_targets[target_no]->skipcnt && j == 0; i++)
        if (strcmp(hydra_heads[head_no]->current_login_ptr, hydra_targets[target_no]->skiplogin[i]) == 0)
          j = 1;
      if (j) {
        if (is_redo == 0) {
          hydra_brains.sent++;
        ...
        if (debug)
          printf("[DEBUG] double found for %s == %s, skipping\n", hydra_heads[head_no]->current_login_ptr, hydra_targets[target_no]->skiplogin[i]);
        return hydra_send_next_pair(target_no, head_no);  // little trick to keep the code small
      }
    }
    
    memset(&buf, 0, sizeof(buf));
    strncpy(buf, hydra_heads[head_no]->current_login_ptr, MAXLINESIZE - 3);
    if (strlen(hydra_heads[head_no]->current_login_ptr) > MAXLINESIZE - 3)
      buflen = MAXLINESIZE - 2;
    else
      buflen = strlen(hydra_heads[head_no]->current_login_ptr) + 1;
    strncpy(buf + buflen, hydra_heads[head_no]->current_pass_ptr, MAXLINESIZE - buflen - 1);
    if (strlen(hydra_heads[head_no]->current_pass_ptr) > MAXLINESIZE - buflen - 1)
      buflen += MAXLINESIZE - buflen - 1;
    else
      buflen += strlen(hydra_heads[head_no]->current_pass_ptr) + 1;
    if (is_redo == 0) {
      hydra_brains.sent++;
      hydra_targets[target_no]->sent++;
    } else
      if (debug)
        printf("[DEBUG] send_next_pair done %d, pass_state %d, clogin %s, cpass %s, tlogin %s, tpass %s, is_redo %d\n",
               done, hydra_targets[target_no]->pass_state, hydra_heads[head_no]->current_login_ptr, hydra_heads[head_no]->current_pass_ptr, hydra_targets[target_no]->login_ptr, hydra_targets[target_no]->pass_ptr, is_redo);
    fck = write(hydra_heads[head_no]->sp[0], buf, buflen);
    if (fck < buflen) {
      if (verbose)
    ...
    if (debug || hydra_options.showAttempt) {
      printf("[%sATTEMPT] target %s - login \"%s\" - pass \"%s\" - %lu of %lu [child %d]\n",
             is_redo ? "RE-" : "", hydra_targets[target_no]->target, hydra_heads[head_no]->current_login_ptr, hydra_heads[head_no]->current_pass_ptr,
...
int hydra_select_target() {
  ...
  for (i = 0; i < hydra_brains.targets; i++)
    if (hydra_targets[i]->use_count < hydra_options.max_use - hydra_targets[i]->use_count)
      if (j < hydra_options.max_use - hydra_targets[i]->failed - hydra_targets[i]->use_count) {
        target_no = i;
        j = hydra_options.max_use - hydra_targets[i]->failed - hydra_targets[i]->use_count;
      }
  return target_no;
}
...
int main(int argc, char *argv[]) {
  ...
  strcat(unsupported, "SSL-services (sip, rdp, oracle-services, ...) ");
#endif
#ifndef HAVE_MATH_H
  if (strlen(unsupported) > 0)
  ...
  if (argc < 3 && (argc < 2 || strcmp(argv[1], "-R") != 0))
    help();
  while ((i = getopt(argc, argv, "64Rde:vVl:fg:L:p:P:o:M:C:t:T:m:w:W:s:SUux:")) >= 0) {
    switch (i) {
    case 'u':
  ...
  if (hydra_options.restore) {
    ...
    if (hydra_options.infile_ptr != NULL) {
      ...
      if ((targetdef != NULL) && (strstr(targetdef, "://") != NULL)) {
        ...
        if (debug)
          fprintf(stderr, "[DEBUG] opt:%d argc:%d mod:%s tgt:%s port:%d misc:%s\n", optind, argc, hydra_options.service, hydra_options.server, hydra_options.port,
    ...
    if (strcmp(hydra_options.service, "ssl") == 0 || strcmp(hydra_options.service, "www") == 0 || strcmp(hydra_options.service, "http") == 0
      ...
      if (strcmp(hydra_options.service, "http") == 0) {
        hydra_options.service = "http-get";
      }
      if (strcmp(hydra_options.service, "https") == 0) {
        hydra_options.service = "https-get";
      }
    }
    ...
    i = 0;
    if (strcmp(hydra_options.service, "telnet") == 0) {
      printf("[WARNING] telnet is by its nature unreliable to analyze reliable, if possible better choose FTP or SSH if available\n");
      i = 1;
    }
    if (strcmp(hydra_options.service, "ftp") == 0)
      i = 1;
    ...
    if ((strcmp(hydra_options.service, "oracle-listener") == 0) || (strcmp(hydra_options.service, "tns") == 0)) {
      i = 2;
      hydra_options.service = "oracle-listener";
    }
    if ((strcmp(hydra_options.service, "oracle-sid") == 0) || (strcmp(hydra_options.service, "sid") == 0)) {
      i = 3;
      hydra_options.service = "oracle-sid";
    }
#ifdef LIBORACLE
    if ((strcmp(hydra_options.service, "oracle") == 0) || (strcmp(hydra_options.service, "ora") == 0)) {
      i = 1;
      hydra_options.service = "oracle";
    }
#endif
    if (strcmp(hydra_options.service, "postgres") == 0)
#ifdef LIBPOSTGRES
...
// ADD NEW SERVICES HERE 
    ...
    if (strcmp(hydra_options.service, "http-get") == 0 || strcmp(hydra_options.service, "http-head") == 0) {
      ...
      if (strcmp(hydra_options.service, "http-head") == 0)
        printf("Warning: http-head auth does not work with every server, better use http-get\n");
    }
    ...
    if (i == 2) {
      if ((hydra_options.login != NULL || hydra_options.loginfile != NULL || hydra_options.colonfile != NULL))
        bail
          ("The cisco, oracle-listener, snmp and vnc modules are only using the -p or -P option, not login (-l, -L) or colon file (-C).\nUse the normal telnet crack mode for cisco using \"Username:\" authentication.\n");
      else
        hydra_options.login = empty_login;
    }
    if (i == 3 && ((hydra_options.pass != NULL && strlen(hydra_options.pass) > 0) || hydra_options.passfile != NULL || hydra_options.colonfile != NULL)) {
      bail("The rsh, oracle-sid login is neither using the -p or -P option nor colon file (-C)\n");
      hydra_options.pass = empty_login;
    }
...
//hydra_dump_data(csv_ptr, hydra_brains.sizelogin + hydra_brains.countlogin + 8, "colon data");
  ...
  if (hydra_options.max_use < hydra_brains.targets)
    hydra_options.max_use = hydra_brains.targets;
  if (hydra_options.max_use > MAXTASKS)
    hydra_options.max_use = MAXTASKS;
  if (hydra_options.max_use < hydra_brains.targets * hydra_options.tasks) {
    if ((hydra_options.tasks = hydra_options.max_use / hydra_brains.targets) == 0)
      hydra_options.tasks = 1;
    fprintf(stderr, "Warning: More tasks defined per servr than allowed for maximal connections. Tasks reduced to %d.\n", hydra_options.tasks);
  } else {
  ...
  printf("%d of %d target%s successfuly completed, %lu valid password%s found\n", hydra_brains.targets - j - k - error, hydra_brains.targets, hydra_brains.targets == 1 ? "" : "s", hydra_brains.found, hydra_brains.found == 1 ? "" : "s");
  if (error == 0 && j == 0) {
    process_restore = 0;
    unlink(RESTOREFILE);
  } else {
